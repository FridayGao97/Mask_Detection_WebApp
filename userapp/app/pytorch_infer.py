...
#if name is same but content is different, append time-stamp at the end treated as different photos, if all same, do nothing
import datetime
...
def runDetection(imgPath):
    ...
    #the filename as first parameter of result
    res.append(output[-1])
    #the photo category as second
    if len(output) == 1:
        print("No Face")
        res.append(0)
    else:
        if maskNum == 0:
            ...
            res.append(3)
    print(output)
    #the number of face and number of masked and number of unmasked as 3rd, 4th, 5th parameter
    res.append(len(output)-1)
    res.append(maskNum)
    res.append(len(output)-1-maskNum)
    print(res)
    return res
